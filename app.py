...
st.set_page_config(page_title="A/Bテスト効果検証ツール（修正中）", layout="wide")
...
msg_go  = "最高です！！今回の施策は効果が出た可能性が高く、再現性が期待できる結果です！単なる偶然ではなく、狙った効果が現れたと考えられます。"
msg_bad = "残念ですが、今回の結果では効果があったと断言できません。効果がゼロとは限りませんが、再現性は不明です。"
msg_low = "データ量が少ないかも！今回の結果では効果があったと断言できません。"
...
with st.expander("判定の考え方"):
    st.markdown("""
- **GO（導入推奨）**：テスト群のCVRがコントロール群より**高い**／差が**意味のある差（MDE）以上**／データ量が**十分**で**偶然のブレではなさそう**。  
- **HOLD（保留）**：データ量が**まだ足りない**（バナーに**必要セッション/不足/日数**を表示） **または** 差は出ているが**小さい**（MDE未満）。  
- **NO-GO（見送り）**：テストのCVRがコントロールより**低いことがはっきり**している（偶然ではなさそう） **または** 差がほとんど見えず、**必要なデータ量は満たしている**。
""")
